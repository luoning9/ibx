...
持仓情况页要求：
- 展示当前账户持仓总览（净值、可用资金、更新时间）。
- 展示持仓明细列表（`sec_type`、`symbol`、`position_qty`、`avg_price`、`last_price`、`market_value/notional`、`unrealized_pnl`）。

交易指令页要求：
- 展示“当前有效交易指令”列表（仅未终态指令）。
- 同页汇总所有策略的“交易核验 + 执行”记录。
- 记录列表必须包含 `trade_id`，用于串联核验、下单、成交等记录。
